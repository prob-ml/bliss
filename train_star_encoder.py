...
#                                     psfield_file,
#                                     band = bands[i])
init_psf_params = torch.Tensor(np.load('./fits/true_powerlaw_psf_params.npy'))
...
init_background_params = torch.Tensor(np.load('./fits/true_planarback_params.npy'))
...
for epoch in range(n_epochs):
    ...
    if (epoch % print_every) == 0:
        ...
        outfile = './fits/results_2020-02-06/starnet_ri_trueback_truepsf'
        print("writing the encoder parameters to " + outfile)
        torch.save(star_encoder.state_dict(), outfile)

        test_losses[:, epoch // print_every] = np.array([test_loss, test_counter_loss, test_locs_loss, test_fluxes_loss])
        np.savetxt('./fits/results_2020-02-06/test_losses-starnet_ri_trueback_truepsf', test_losses)
