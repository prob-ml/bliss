...
class TestStarEncoder(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        n_star_patches = torch.Tensor(np.random.choice(max_detections, n_image_patches)).type(torch.LongTensor)

        # forward
        loc_mean, loc_logvar, \
            log_flux_mean, log_flux_logvar, log_probs = \
                star_encoder(image_patches, n_star_patches)
        ...
        is_on_array = utils.get_is_on_from_n_stars(n_star_patches, star_encoder.max_detections)
        ...
        loc_mean, loc_logvar, \
            log_flux_mean, log_flux_logvar, log_probs = \
                star_encoder(image_patches, n_stars = None)
