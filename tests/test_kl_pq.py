...
class TestStarEncoderObjective:
    ...
    def test_get_min_perm_loss(self):
        ...
        true_galaxy_params = torch.randn(
            n_ptiles, max_detections, n_galaxy_params, device=device
        ) * true_is_on_array.unsqueeze(2)
        ...
        galaxy_params_mean = torch.randn(
            n_ptiles, max_detections, n_galaxy_params, device=device
        ) * true_is_on_array.unsqueeze(2)
        galaxy_params_logvar = torch.randn(
            n_ptiles, max_detections, n_galaxy_params, device=device
        ) * true_is_on_array.unsqueeze(2)
        ...
        assert (galaxy_params_loss[true_n_sources == 0] == 0).all()
        ...
        # when there are no galaxies: galaxy loss should be zero
        which_no_gal = (true_galaxy_bool * true_is_on_array).sum(1) == 0
        assert (galaxy_params_loss[which_no_gal] == 0).all()
        ...
        for i in range(n_ptiles):
            ...
            _true_galaxy_params = true_galaxy_params[i, 0:_true_n_sources, :]
            _galaxy_params_mean = galaxy_params_mean[i, 0:_true_n_sources, :]
            _galaxy_params_logvar = galaxy_params_logvar[i, 0:_true_n_sources, :]
            ...
            for perm in permutations(range(_true_n_sources)):
                ...
                galaxy_params_loss_perm = (
                    -Normal(
                        _galaxy_params_mean[perm, :],
                        (torch.exp(_galaxy_params_logvar[perm, :]) + 1e-5).sqrt(),
                    )
                    .log_prob(_true_galaxy_params)
                    .sum(-1)
                )
                ...
                if locs_loss_perm.sum() < min_locs_loss:
                    ...
                    min_galaxy_params_loss = (
                        galaxy_params_loss_perm * _true_galaxy_bool
                    ).sum()
            ...
            assert (
                torch.abs(star_params_loss[i] - min_star_params_loss) < 1e-5
            ), torch.abs(star_params_loss[i] - min_star_params_loss)
