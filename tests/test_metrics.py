...
class TestMetrics:
    ...
    def test_metrics(self):
        ...
        acc_metrics = SourceTypeAccuracy(base_njy_bin_cutoffs=[200, 400, 600, 800, 1000])
        acc_results = acc_metrics(true_params, est_params, matching)
        assert np.isclose(acc_results["classification_acc"], 1 / 2)

        gal_shape_metrics = GalaxyShapeError(base_njy_bin_cutoffs=[200, 400, 600, 800, 1000])
    ...
    def test_self_agreement(self, tile_catalog):
        ...
        flux_metrics = FluxError("ugriz", base_njy_bin_cutoffs=[200, 400, 600, 800, 1000])
