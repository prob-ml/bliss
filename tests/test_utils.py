...
class TestUtils:
    def test_get_one_hot(self):
        ...
        n_classes = 10
        z = torch.randint(0, 10, (100,)).to(utils.device)
    ...
    def test_is_on_from_n_stars(self):
        max_stars = 10
        n_stars = torch.Tensor(np.random.choice(max_stars, 5)).to(utils.device)

        is_on = utils.get_is_on_from_n_sources(n_stars, max_stars)
    ...
    def test_is_on_from_n_stars2d(self):
        ...
        is_on = utils.get_is_on_from_tile_n_sources_2d(n_stars, max_stars)

        for i in range(n_samples):
            assert torch.all(
                utils.get_is_on_from_n_sources(n_stars[i], max_stars) == is_on[i]
            )
