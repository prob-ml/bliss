...
class SourceSleep(object):
    def __init__(self, encoder, dataset, n_epochs, n_source_params,
        ...
        self.prob_func_source_param = None  # function return probability given source params in Tensor form.
    ...
    def eval_sleep(self, train=False):
        ...
        for i in range(num_batches):
            ...
            true_source_params, true_locs, images = self.get_params_from_data(data)
    ...
    def _get_params_loss(self, loc_mean, loc_logvar,
        ...
        * source_param_mean, source_param_logvar = log_flux_mean, log_flux_logvar.
        ...
        # the log prob for each observed location x mean
        locs_log_probs_all = _get_locs_logprob_all_combs(true_locs,
                                                         loc_mean,
                                                         loc_logvar)

        flux_log_probs_all = self._get_source_params_logprob_all_combs(true_source_params, source_param_mean,
                                                                       source_param_logvar)

        locs_loss, fluxes_loss, perm_indx = get_min_perm_loss(locs_log_probs_all, flux_log_probs_all, true_is_on_array)
