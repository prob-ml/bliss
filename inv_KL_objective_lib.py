...
def get_encoder_loss(star_encoder,
    ...
    is_on_array = is_on_array * (is_on_array.sum(dim = 1) < star_encoder.max_detections).unsqueeze(1)
    perm = run_batch_hungarian_alg_parallel(locs_log_probs_all, is_on_array).to(device)
