...
from bliss.base_detect.utils.autoanchor import check_anchor_order
from bliss.base_detect.utils.general import (
    LOGGER,
    check_version,
    check_yaml,
    colorstr,
...
from bliss.base_detect.utils.torch_utils import (
    fuse_conv_and_bn,
    initialize_weights,
    model_info,
    profile,
...
class Detect(torch.nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # output conv
        self.m = torch.nn.ModuleList(torch.nn.Conv2d(x, self.no * self.na, 1) for x in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if isinstance(self):  # (boxes + masks)
                    xy, wh, conf, mask = x[i].split((2, 2, self.nc + 1, self.no - self.nc - 5), 4)
                    xy = (xy.sigmoid() * 2 + self.grid[i]) * self.stride[i]  # pylint: disable=E1136
                    wh = (wh.sigmoid() * 2) ** 2 * self.anchor_grid[i]
                    y = torch.cat((xy, wh, conf.sigmoid(), mask), 4)
                else:  # Detect (boxes only)
                    xy, wh, conf = x[i].sigmoid().split((2, 2, self.nc + 1), 4)
                    xy = (xy * 2 + self.grid[i]) * self.stride[i]  # pylint: disable=E1136
        ...
        return (
            x
            if self.training
            else (torch.cat(z, 1),)  # noqa: WPS509
            if self.export
    ...
    def _make_grid(
        ...
        ny=20,
        i=0,
        torch_1_10=check_version(torch.__version__, "1.10.0"),  # noqa: WPS404, WPS114, B008
    ):  # noqa: WPS114, B008, WPS404
        ...
        anchor_grid = (
            (self.anchors[i] * self.stride[i])  # pylint: disable=E1136
            .view((1, self.na, 1, 1, 2))  # noqa: WPS348
            .expand(shape)  # noqa: WPS348
        )
...
class BaseModel(torch.nn.Module):
        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                x = (
                    ...
                    else [x if j == -1 else y[j] for j in m.f]  # noqa: WPS509
                )  # from earlier layers
            if profile:
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        o = (
            thop.profile(m, inputs=(x.copy() if c else x,), verbose=False)[0]  # noqa: WPS509
            / 1e9
            * 2  # noqa: WPS509
            if thop
            else 0
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, (Conv)) and hasattr(m, "bn"):  # noqa: WPS421
                m.conv = fuse_conv_and_bn(m.conv, m.bn)  # update conv
                delattr(m, "bn")  # remove batchnorm    # noqa: WPS421
    ...
    def _apply(self, fn):
        ...
        self = super()._apply(fn)  # pylint: disable=W0642  # noqa: WPS117
...
if __name__ == "__main__":
    ...
    print_args(vars(opt))  # noqa: WPS421
    ...
    if opt.line_profile:  # profile layer by layer
        ...
        for cfg in Path(ROOT / "models").rglob("yolo*.yaml"):
            try:
                _ = Model(cfg)  # noqa: WPS122
            except Exception as e:  # pylint: disable=W0718
                print(f"Error in {cfg}: {e}")  # noqa: WPS421
