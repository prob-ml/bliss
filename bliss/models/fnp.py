...
def make_fc_skip(insize, hs, outsize, act=nn.ReLU, final=None, size_skip=1, skip_final=False):
    layers = [linear_skip_connection(insize, hs[0], size_skip=size_skip), act()]
    for i in range(len(hs)-1):
        layers += [linear_skip_connection(hs[i]+size_skip, hs[i+1], size_skip=size_skip), act()]
    if skip_final:
        layers += [linear_skip_connection(hs[-1]+size_skip, outsize, size_skip=size_skip)]
    else:
...
class RegressionFNP(pl.LightningModule):
    ...
    def __init__(self, dim_x=1, dim_y=1, dim_h=50, transf_y=None, n_layers=1, use_plus=True, num_M=100,
                 dim_u=1, dim_z=1, fb_z=0., G=None, A=None, y_encoder_layers=[128], mu_nu_layers=[128], use_x_mu_nu=True, 
                 use_direction_mu_nu=False, mu_nu_skip=False,
                 output_layers=[128], x_as_u=False, condition_on_ref=False, train_separate_proposal=True, train_separate_extrapolate=False, discrete_orientation=True,
                 weighted_graph=False, y_encoder_resid=False, mu_nu_resid=False, output_resid=False):
        '''
    ...
    def make_trans_cond_y(self):
        ...
        return make_fc_net(self.dim_y, self.y_encoder_layers, 2 * self.dim_z, resid=self.y_encoder_resid)
    ...
    def make_mu_nu_theta(self):
        ...
        return make_fc_net(mu_nu_in, self.mu_nu_layers, 2 * self.dim_z, resid=self.mu_nu_resid)
    ...
    def make_output(self):
        ...
        return make_fc_net(output_insize, self.output_layers, 2 * self.dim_y, resid=self.output_resid)
    ...
    def calc_pz_dist(self, u, uR, GA, XR, yR, minscale=1e-8):
        ...
        if not self.use_direction_mu_nu:
            if not self.use_x_mu_nu:
                ...
                mu_nu_in = torch.cat([
                    yR_encoded, 
                    XR.unsqueeze(0).repeat(yR_encoded.size(0), 1, 1)], dim=-1)
        else:
            mu_nu_in = torch.cat([
                yR_encoded.unsqueeze(1).repeat(1, pairwise_differences.size(0), 1, 1),
                pairwise_differences.unsqueeze(0).repeat(yR_encoded.size(0), 1, 1, 1)], dim=3)
        ...
        if self.train_separate_extrapolate:
            ...
            mu_nu_in = torch.cat([
                mu_nu_in,
                extrapolating.float().unsqueeze(1).unsqueeze(0).repeat(mu_nu_in.size(0), 1, mu_nu_in.size(2), 1)], dim=3)

        pz_mean_R, pz_logscale_R  = torch.split(self.mu_nu_theta(mu_nu_in), self.dim_z, -1)
        ...
        pz_logscale_all = torch.log(minscale + (1-minscale) * F.softplus(pz_logscale_all))
    ...
    def calc_qz_dist(self, X_all, y_all, minscale=1e-8):
        ...
        if self.use_x_mu_nu:
            y_all_encoded = torch.cat([y_all_encoded, X_all.unsqueeze(0).repeat(y_all_encoded.size(0), 1, 1)], dim=-1)
        if self.use_direction_mu_nu and not self.train_separate_proposal:
            y_all_encoded = torch.cat([y_all_encoded, torch.zeros(*y_all_encoded.shape[0:2], 1, device=y_all_encoded.device)], dim=2)
        qz_mean_all, qz_logscale_all = torch.split(self.mu_nu_proposal(y_all_encoded), self.dim_z, -1)
        qz_logscale_all = torch.log(minscale + (1-minscale) * F.softplus(qz_logscale_all))
    ...
    def calc_py_dist(self, z, u):
        final_rep        = z if not self.use_plus else torch.cat([z, u.unsqueeze(0).repeat(z.size(0), 1, 1)], dim=-1)
        # Encodes y from z
        mean_y, logstd_y  = self.calc_output_y(final_rep)
        return mean_y, logstd_y

    def forward(self, XR, yR, XM, yM, G_in=None, A_in=None, kl_anneal=1.):
        n_ref = XR.size(0)
        ...
        logstd_yR, logstd_yM = torch.split(logstd_y, [n_ref, logstd_y.size(1) - n_ref], 1)
...
class PoolingFNP(RegressionFNP):
    def __init__(self, pooling_layers=[64], pooling_rep_size=32, pooling_resid=False, set_transformer=False, st_numheads=[2,2], **kwargs):
    ...
    def make_pool_net(self):
        ...
        return make_fc_net(dim_in, self.pooling_layers, 2 * self.dim_z, resid=self.pooling_resid)

    def make_settrans(self):
        dim_in   = self.mu_nu_theta[-1].out_features
        sabs     = [SAB(dim_in, dim_in, nh) for nh in self.st_numheads]
        settrans = nn.Sequential(*sabs, 
            PMA(dim_in, 2, 1, squeeze_out=True), 
            make_fc_net(dim_in, self.pooling_layers, 2*self.dim_z, resid=self.pooling_resid))
        return settrans
    ...
    def calc_pz_dist(self, u, uR, GA, XR, yR, minscale=1e-8):
        ...
        mu_nu_in = torch.cat([
            ...
            u_diff.unsqueeze(0).repeat(yR_encoded.size(0), 1, 1, 1)], dim=-1)
...
class ConvRegressionFNP(RegressionFNP):
    ...
    """
    def __init__(self, size_h=10, size_w=10, kernel_sizes=[3,3], strides=[1,1], conv_channels=[20,20], **kwargs):
        self.size_h     = size_h
        self.size_w     = size_w
    ...
    def make_trans_cond_y(self):
        ...
        q, dummy_input, h_out, w_out, pad_h, pad_w = make_conv_layer_and_trace(1, self.conv_channels[0], self.kernel_sizes[0], self.strides[0], dummy_input)
        y_encoder_array = [q, nn.ReLU()]
        self.pad_hs.append(pad_h)
        self.pad_ws.append(pad_w)
        for i in range(len(self.conv_channels)-1):
            q, dummy_input, h_out, w_out, pad_h, pad_w = make_conv_layer_and_trace(self.conv_channels[i], self.conv_channels[i+1], self.kernel_sizes[i+1], self.strides[i+1], dummy_input)
            y_encoder_array += [q, nn.ReLU()]
            self.pad_hs.append(pad_h)
            self.pad_ws.append(pad_w)
    ...
    def calc_trans_cond_y(self, yR):
        yR_encoded = self.trans_cond_y(yR.view(yR.size(0)*yR.size(1), yR.size(2), yR.size(3), yR.size(4)))
    ...
    def make_output(self):
        ...
        for i in range(len(self.conv_channels) - 1):
            ...
            output_array.append(nn.ConvTranspose2d(
                inchannel, 
                ouchannel,
        ...
        output_array += [nn.ReLU(), nn.ConvTranspose2d(self.conv_channels[0], 2, self.kernel_sizes[0], self.strides[0])]

        return nn.Sequential(*output_array)

    def calc_output_y(self, final_rep):
        output_y         = self.output(final_rep.view(final_rep.size(0) * final_rep.size(1), final_rep.size(2)))
        output_y         = output_y.view(final_rep.size(0), final_rep.size(1), output_y.size(1), output_y.size(2), output_y.size(3))
        mean_y           = output_y[:,:,0:1,:,:]
...
class L1Error(object):
    ...
    def log_prob(self, value):
        ...
        log_prob *= - (1 / (2. * self.logscales.mul(1.).exp()))
        log_prob -= self.logscales + .5 * math.log(2. * math.pi)
        return log_prob

    def sample(self, **kwargs):
        eps = torch.distributions.Laplace(float_tensor(self.means.size()).zero_(), float_tensor(self.means.size()).fill_(1)).sample()
        return self.means + self.logscales.exp() * eps
