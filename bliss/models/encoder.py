...
class ImageEncoder(nn.Module):
    def __init__(
        ...
        * For fluxes this should equal number of bands, for galaxies it will be the number of latent
        ...
        self.tile_coords = self._get_tile_coords(
            slen, slen, self.ptile_slen, self.tile_slen
        )
    ...
    def _get_tile_coords(self, image_xlen, image_ylen, ptile_slen, tile_slen):
        """
    ...
    def get_params_in_tiles(self, slen, locs, *params):
        ...
        tile_coords = self._get_tile_coords(slen, slen, self.ptile_slen, self.tile_slen)

        return self._get_params_in_tiles(
            tile_coords,
            self.max_detections,
            slen,
            self.edge_padding,
    ...
    def _get_full_params_from_sampled_params(
        ...
        _indx_sort = self._argfront(locs[..., 0], dim=1)
    ...
    def sample_encoder(self, image, n_samples):
        ...
        tile_n_sources = self._sample_class_weights(probs_n_sources_per_tile, n_samples)
    ...
    def _argfront(self, is_on_array, dim):
    ...
    def _sample_class_weights(self, class_weights, n_samples=1):
        """
    ...
    def _clip_params(self, max_detections, tile_n_sources, *tiled_params):
    ...
    def _tile_locs(self, tile_coords, slen, edge_padding, ptile_slen, locs):
        ...
        tile_is_on_array = tile_is_on_array.float()
        ...
        indx_sort = self._argfront(tile_is_on_array, dim=1)
    ...
    def _get_tile_params(self, tile_is_on_array, indx_sort, params):
    ...
    def _get_params_in_tiles(
        self, tile_coords, max_detections, slen, edge_padding, ptile_slen, locs, *params
    ):
        (
            tile_n_sources,
            tile_locs,  # sorted.
            ...
            indx_sort,
        ) = self._tile_locs(tile_coords, slen, edge_padding, ptile_slen, locs)

        # now sort the rest of the parameters
        tile_params = self._get_tile_params(_tile_is_on_array, indx_sort, params)

        # now that we are done we can sort this tensor too.
        tile_is_on_array = torch.gather(_tile_is_on_array, 1, indx_sort)

        # clip if necessary.
        (tile_n_sources, *tile_params, tile_is_on_array) = self._clip_params(
