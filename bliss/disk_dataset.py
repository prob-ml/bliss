...
class DiskDataset(pl.LightningDataModule, Dataset):
    ...
    def __init__(
        ...
        if len(self.data_files) != self.disk_n_batches:
            ...
            for disk_batch_idx in range(self.disk_n_batches):
                ...
                with open(f"{self.data_path}/dataset_{disk_batch_idx}.pkl", "wb") as data_file:
    ...
    def read_batch(self, disk_batch_idx) -> Dict:
        ...
        batch_data = None
        with open(
            f'{self.data_path}/{self.data_files[disk_batch_idx]["filename"]}', "rb"
        ) as data_file:
            try:
                batch_data = pickle.load(data_file)
            except EOFError as eof:
                print(
                    f'Error: EOFError while reading {self.data_files[disk_batch_idx]["filename"]}',
                )
    ...
    def __getitem__(self, idx):
        ...
        disk_batch_data = self.data_files[disk_batch_idx]["data"]
        tile_catalog_dict = {}
        for key in disk_batch_data["tile_catalog"].keys():
            tile_catalog_dict[key] = disk_batch_data["tile_catalog"][key][idx_in_batch]
        image = disk_batch_data["images"][idx_in_batch]
        background = disk_batch_data["background"][idx_in_batch]
        return {"tile_catalog": tile_catalog_dict, "images": image, "background": background}

    def train_dataloader(self):
        return DataLoader(
            self, batch_size=self.image_prior.batch_size, num_workers=self.num_workers
        )
