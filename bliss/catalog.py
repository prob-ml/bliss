...
class TileCatalog(UserDict):
    ...
    def __eq__(self, other):
        ...
        return (
            self.tile_slen == other.tile_slen
            and torch.equal(self.locs, other.locs)
            and torch.equal(self.n_sources, other.n_sources)
...
class FullCatalog(UserDict):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and torch.equal(self.plocs, other.plocs)
            and torch.equal(self.n_sources, other.n_sources)
            and self.batch_size == other.batch_size
            and self.max_sources == other.max_sources
            and self._test_data_equal(other.data)
    ...
    def to_tile_catalog(
        ...
        for ii in range(self.batch_size):
            ...
            tile_to_source_mapping = self._get_tile_to_source_mapping(
                (source_indices == tile_indices).long()
            )
            ...
            for k, v in tile_params.items():
                ...
                )  # the second argument of pad function is (padding_left, padding_right, padding_top, padding_bottom)
