...
class SimulatedGenerator:
    def __init__(self, batch_size: int, decoder_args, decoder_kwargs):
        super(SimulatedGenerator, self).__init__()
    ...
    def get_image(self):
        params = self.image_decoder.sample_prior(self.batch_size)
    ...
    def decoder_args_from_args(args, paths: dict, device):
        ...
        dec = SimulatedGenerator.get_gal_decoder_from_file(
            decoder_file, gal_slen, n_bands, latent_dim, device
        )
        background = SimulatedGenerator.get_background_from_file(
            background_file, slen, n_bands
        )
        psf_params = SimulatedGenerator.get_psf_params_from_file(psf_file, device)[
            range(n_bands)
        ]
    ...
    def from_args(cls, args, paths: dict, device):
        ...
        decoder_args = SimulatedGenerator.decoder_args_from_args(args, paths, device)
        decoder_kwargs = {
            key: value
...
class SimulatedDataset(Dataset):
    def __init__(self, n_batches: int, batch_size: int, decoder_args, decoder_kwargs):
        ...
        self.batch_iterator = SimulatedGenerator(
            batch_size, decoder_args, decoder_kwargs
        )
        self.index = 0
    ...
    def __getitem__(self, item):
        get_next = next(self.batch_iterator)
        return get_next
