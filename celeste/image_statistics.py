...
def get_tpr_vec(
    ...
    if mag_vec is None:
        ...
        max_mag = torch.ceil(true_mags.max() * 2) / 2
        min_mag = torch.floor(true_mags.min() * 2) / 2
...
def get_ppv_vec(
    ...
    if mag_vec is None:
        max_mag = torch.ceil(est_mags.max() * 2) / 2
        min_mag = torch.floor(est_mags.min() * 2) / 2
...
def get_l1_error(est_locs, true_locs, slen, est_fluxes, true_fluxes, pad=5):
    ...
    seq_tensor = torch.Tensor([i for i in range(fluxes_matched_error.shape[1])]).type(
        torch.long
    )
