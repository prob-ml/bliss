...
from bliss.models.galsim_decoder import FullCatalogDecoder, PsfSampler, UniformGalsimPrior
...
class GalsimBlendswithPSF(GalsimBlends):
    ...
    def _get_images(self, full_cat, psf):
        ...
        psf = torch.from_numpy(
            psf.drawImage(nx=self.slen, ny=self.slen, scale=self.pixel_scale).array
    ...
    def __getitem__(self, idx):
        psf_obj = self.psf_sampler.get_galsim_psf()
        full_cat = self._sample_full_catalog()
        images, *metric_images, background, homo_image, psf = self._get_images(full_cat, psf_obj)
        full_cat = self._add_metrics(full_cat, *metric_images, background, psf_obj)
