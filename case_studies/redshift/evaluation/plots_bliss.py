...
def main(cfg: DictConfig):
    ...
    metrics = ['outlier_fraction_cata', 'outlier_fraction',
               'nmad', 'bias_abs', 'mse']
    metric_labels = ['Catastrophic Outlier Fraction', 'Outlier Fraction',
                     'NMAD', 'Absolute Bias', 'MSE']
    sns.set_theme()
    for i, metric in enumerate(metrics):
        save_name = output_dir / f"{metric}.pdf"

        mag_ranges = ['<23.9', '23.9-24.1', '24.1-24.5',
        ...
        bliss_discrete_grid = [bliss_discrete_out_dict[f'redshifts/{metric}_bin_{i}']
                               for i in range(6)]

        plt.figure(figsize=(6, 6))
        plt.plot(mag_ranges, bliss_values, label="BLISS+Normal", marker='o', c="blue")
        plt.plot(mag_ranges, bliss_discrete,
                 label="BLISS+Discrete Bin", marker='o', c="green")
