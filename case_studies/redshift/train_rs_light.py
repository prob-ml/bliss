...
def main(epoch, train_path, val_path, nick, resume, outdir):
    ...
    dataloader_train = preprocess(photo_z_train, dataset_options['batch_size'], dataset_options['group_size'], dataset_options['num_bins'], dataset_options['x_dim'], dataset_options['y_dim'])
    dataloader_val = preprocess(photo_z_val, dataset_options['batch_size'], dataset_options['group_size'], dataset_options['num_bins'], dataset_options['x_dim'], dataset_options['y_dim'])
    ...
    network_options = {
        ...
        'loss_fcn': torch.nn.MSELoss() if dataset_options['num_bins']==1 else torch.nn.CrossEntropyLoss(),           # loss func
        'dropout_rate': 0.5,
        'learning_rate': 1e-3,
        ...
        'val_check_interval': 0.2 # How often to check the validation set. Pass a float in the range [0.0, 1.0] to check after a fraction of the training epoch.
    }
    in_dim = dataset_options['x_dim']
    reg = PhotoZFromFluxes(in_dim, network_options['hidden_dim'], network_options['out_dim'], network_options['dropout_rate'], network_options['learning_rate'], network_options['loss_fcn'])
...
def preprocess(df, batch_size, group_size=1, num_bins=1, x_dim=6, y_dim=1):
    """
    inputs:
        df: Dataframe
        batch_size:
        group_size: size for each group
    returns:
        out: dataloader
    """
    ...
    if group_size != 1:
        ...
        # tensor_x = torch.from_numpy(x_train).view(-1, group_size, n_features_x)
        # tensor_y = torch.from_numpy(y_train).view(-1, group_size, n_features_y)
    else:
        tensor_x = torch.Tensor(x_train)
        tensor_y = torch.Tensor(y_train)
    custom_dataset = TensorDataset(tensor_x, tensor_y)
    dataloader = DataLoader(custom_dataset, batch_size=batch_size, pin_memory=True)
