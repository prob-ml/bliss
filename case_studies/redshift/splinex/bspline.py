...
class DeclanBSpline:
    ...
    def __init__(
        ...
        self.knots = torch.cat(
            [self.knots.new_full((degree,), min_val), self.knots, self.knots.new_full((degree,), max_val)]
        )
    ...
    def compute_derivative_matrix(self, order: int = 1) -> torch.Tensor:
        ...
        derivative_matrix = torch.stack([
            torch.stack([self.derivative_basis_function(t, self.degree, i, order) for t in self.t_values])
            for i in range(self.n)
        ])
    ...
    def basis_function(self, t: float, k: int, i: int) -> float:
        ...
        if k == 0:
            ...
            if self.knots[i + k] != self.knots[i]:
                left_term = (t - self.knots[i]) / (self.knots[i + k] - self.knots[i]) * self.basis_function(t, k - 1, i)
            
            right_term = 0.0
            if self.knots[i + k + 1] != self.knots[i + 1]:
                right_term = (self.knots[i + k + 1] - t) / (self.knots[i + k + 1] - self.knots[i + 1]) * self.basis_function(t, k - 1, i + 1)

            special_case = 1.0 if not ((i == 0 and t == self.knots[0]) or (i == self.n - 1 and t == self.knots[-1])) else 0.0
    ...
    def derivative_basis_function(
        ...
        if order == 0:
            ...
            left_term = 0.0
            if self.knots[i + k] != self.knots[i]:
                left_term = k / (self.knots[i + k] - self.knots[i]) * self.derivative_basis_function(t, k - 1, i, order - 1)
            
            right_term = 0.0
            if self.knots[i + k + 1] != self.knots[i + 1]:
                right_term = k / (self.knots[i + k + 1] - self.knots[i + 1]) * self.derivative_basis_function(t, k - 1, i + 1, order - 1)
