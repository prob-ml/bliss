...
class ToyGalaxyClusterDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            print(f"Error loading subtile {self.sub_file_paths[idx]}: {e}")
...
class GalaxyClusterDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ### Path to the dataset folder containing the .pt files
...
def run_training():
    ...
    ### Call backs and checkpoints saving
    checkpoint_cb = ModelCheckpoint(
        monitor="val_loss",        # what to monitor
        mode="min",                # lower is better
        save_top_k=5,              # keep only the best one so far
        filename="best-{epoch}-{val_loss:.4f}",
        dirpath="checkpoints_812_real/",    # where to save
    )
    ...
    ### Training on a previously saved checkpoint
    trainer.fit(model, data_module, ckpt_path="/home/hughwang/bliss/bliss/checkpoints_731_real/best-epoch=49-val_loss=0.4247.ckpt")
