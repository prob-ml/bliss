...
def compress_shear(shear_tensor):
    """Compress shear tensor without averaging (preserves original range).
    ...
    Args:
        shear_tensor: Input tensor with shape (n_tiles, n_tiles, max_sources, 1) or similar.

    Returns:
        Compressed tensor with shape (n_tiles, n_tiles, 1).
    """
...
def find_batch_files(batch_dir, pattern="*"):
    ...
    Raises:
        FileNotFoundError: If batch directory does not exist.
    """
...
def pair_image_catalog_batches(image_dir, catalog_dir, image_pattern="*", catalog_pattern="*"):
    ...
    if len(image_files) != len(catalog_files):
        logger.warning(
            f"Mismatch in number of files: {len(image_files)} images vs "
            f"{len(catalog_files)} catalogs"
        )
...
class MemoryControlledBatchProcessor:
    ...
    def process_single_batch(self, image_path, catalog_path, batch_id, save_folder):
        ...
        try:
            ...
            logger.info(
                f"Memory before loading batch: {used_memory:.1f}/"
            ...
            for idx in tqdm(range(num_images), desc=f"Batch {batch_id}", unit="img"):
                try:
                    ...
                    if self.global_image_counter == 0:
                        logger.info("Shear compression results for first image:")
                    ...
                    if (idx + 1) % 10 == 0:
                        ...
                        if percent > 85:
                            logger.warning(
                                f"High memory usage: {used_memory:.1f} GB "
                                f"({percent:.1f}%) - forcing cleanup"
                            )
                            self.cleanup_memory()
...
def process_multiple_batches(
    ...
    try:
        batch_pairs = pair_image_catalog_batches(
    ...
    for batch_idx, (image_path, catalog_path) in enumerate(batch_pairs, start=start_batch):
        ...
        if batch_idx > start_batch:
            ...
            logger.info(
                f"Batch progress: {batch_idx - start_batch + 1}/{len(batch_pairs)} | "
    ...
    logger.info(
        f"Average time per batch: "
        f"{format_time(total_time / total_processed) if total_processed > 0 else 'N/A'}"
    )
    logger.info(
        f"Output files: dataset_0_size_1.pt to "
        f"dataset_{processor.global_image_counter - 1}_size_1.pt"
    )
    logger.info(f"Output folder: {save_folder}")
    logger.info("=" * 60)
...
def main():
    logger.info("=" * 60)
    logger.info(
        f"Starting batch weak lensing data processing at "
