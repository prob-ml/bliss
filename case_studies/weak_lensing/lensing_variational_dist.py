...
class LensingVariationalDist(VariationalDist):
    def sample(self, use_mode=False) -> TileCatalog:
        ...
        est_cat["shear"] = (
            (q["shear"].mode if use_mode else q["shear"].sample()).squeeze(0).unsqueeze(3)
        )
        ecc3d = q["convergence"].mode if use_mode else q["convergence"].sample()
        est_cat["convergence"] = ecc3d.unsqueeze(3).unsqueeze(3)
