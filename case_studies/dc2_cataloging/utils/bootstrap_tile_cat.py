...
def bootstrap_tile_cat(ori_tile_cat: TileCatalog, seed: int, space_preserve=False):
    ...
    random_indices = torch.from_numpy(
        rng.choice(batch_size, (batch_size,), replace=True)
    )
    for k, v in tile_dict.items():
        tile_dict[k] = torch.index_select(v, dim=0, index=random_indices)
    return TileCatalog(tile_dict)
