...
parser = argparse.ArgumentParser(description='Run Point Spread Function (PSF) using Functional Neural Process (FNP).')
parser.add_argument('--device', dest='cuda_device', type=int, nargs=1, help='CUDA device to run on', default=[0])
parser.add_argument('--epochs', dest='epochs', type=int, nargs=1, help='Number of epochs to run through the data', default=[1000])
parser.add_argument('--batchsize', dest='batch_size', type=int, nargs=1, help='Minibatch size for SGD', default=[10])
parser.add_argument('--learnrate', dest='learnrate', type=float, nargs=1, help='Learning rate for ADAM', default=[1e-3])
parser.add_argument('--condref', dest="condition_on_ref", action="store_true")
parser.add_argument('--notrainproposal', dest="train_separate_proposal", action="store_true")
parser.add_argument('--skiptrain', dest="skiptrain", action="store_true")

parser.add_argument('--generate', help="Generate the data rather than read it in", action="store_true")
parser.add_argument("--n_ref", help="(DGP) Number of reference stars to add",type=int, dest="n_ref", nargs=1, default=[50])
parser.add_argument("--N_rows", help="(DGP) Number of independent rows to generate", type=int, dest="N", nargs=1, default=[500])
parser.add_argument("--sizeh", help="(DGP) Height of generated images", type=int, dest="size_h", nargs=1, default=[50])
parser.add_argument("--sizew", help="(DGP) Width of generated images", type=int, dest="size_w", nargs=1, default=[50])
parser.add_argument("--covmultiplier", help="(DGP) How big the star is", type=float, dest="cov_multiplier", nargs=1, default=[20.0])
parser.add_argument("--baseangle", help="(DGP) How fast the stars rotate on average (as a multiple of pi)", type=float, dest="base_angle", nargs=1, default=[(3/23)])
parser.add_argument("--anglestdev", help="(DGP) The standard deviation of rotation (as a multiple of pi; higher means angle is less predictable)", type=float, dest="angle_stdev", nargs=1, default=[0.1])
...
if __name__ == "__main__":
    ...
    if generate:
        ...
        star_data = dgp.PsfFnpData(n_ref, Ks, N, size_h=size_h, size_w=size_w,
            cov_multiplier=cov_multiplier, base_angle=base_angle, angle_stdev=angle_stdev,
            bright_skip=1, star_width=0.1, conv=True, N_valid=10, device="cuda:{0}".format(args.cuda_device[0]))
        ...
        star_data.export_images(outdir+"rotating_star_dgp_valid.png", nrows=10, valid=True)
    ...
    fnp_model = fnp.ConvPoolingFNP(dim_x=1, size_h=size_h, size_w=size_w, kernel_sizes=[3,3,3,3,3], strides=[1,2,2,2,2], conv_channels=[16, 32, 64, 128, 256], 
        transf_y=star_data.stdy, dim_h=100, dim_u=1,
        pooling_layers = [32, 16],
        n_layers=3, num_M=star_data.X_m.size(0),dim_z=8,
        fb_z=1.0, use_plus=False, G=star_data.G, A=star_data.A, mu_nu_layers=[128, 64, 32], use_x_mu_nu=False, use_direction_mu_nu=True, 
        output_layers=[32, 64, 128], x_as_u=True,
                                    condition_on_ref=condition_on_ref, train_separate_proposal=train_separate_proposal, train_separate_extrapolate=False, discrete_orientation=False, set_transformer=True)
    ...
    if not skip_train:
        ...
        for i in range(epochs_outer):
            ...
            for i_n in range(nbatches):
                ...
                loss = fnp_model(star_data.X_r, y_r_in, star_data.X_m, y_m_in) / batch_size
                loss.backward()
                optimizer.step()
                if ((i*N) + i_n) % int(epochs/callback) == 0:
                    print('({:05.1f}%)Epoch {}/{}. loss: {:.3f}'.format(((i*N) + i_n) / epochs * 100,(i*N) + i_n, epochs, loss.item()))
        toc = time.perf_counter()
    ...
    X_extra = star_data.X_ref.max() + torch.tensor([1., 2., 3., 4., 5.]).cpu()
    X_extra_std = torch.from_numpy(star_data.stdx.transform(X_extra.cpu().unsqueeze(1)).astype(np.float32)).cuda()
    ...
    myimg = star_data.make_fnp_mean_image(fnp_model, X=X_pred_std, X_nostd=X_pred, A=A_pred, samples=10, valid=True)
    ...
    plt.imsave(outdir+"rotating_star_fnp_dgp_valid_var.png", myimgvar, vmin=vmin, vmax=vmax)

    many_images = torch.stack([star_data.make_fnp_single_image(fnp_model, valid=True) for i in range(100)])
